...
inline float findC(const vec2& p, const vec2& AB) { // C = -(Ax + By)
	return -dot(p, AB);
}
// Ax + By + C = 0
class LineEquation {
...
public:
	...
	void applyStartT(float t) {
		m_start.offset(m_dx * t, m_dy * t);
		m_dx *= (1 - t);
		m_dy *= (1 - t);
	}
	void applyEndT(float t) {
		m_dx *= t;
		m_dy *= t;
	}
...
inline DLineSeg makeLineSegment(const vec2& in_start, const vec2& in_end) {
